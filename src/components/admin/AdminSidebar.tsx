...
const NAV_ITEMS = [
  ...
  { label: "Manage Testers", href: "/admin/testers", icon: "users" },
  { label: "Manage Issues", href: "/admin/issues", icon: "list" },
  ...
  const sidebar = (
    <div className="flex flex-col h-full">
        ...
        <Link href="/admin" className="flex items-center gap-3">
          ...
          <div>
            <span className="text-white font-semibold text-sm">
              <em className="italic text-brand-yellow not-italic font-semibold" style={{ fontStyle: "italic" }}>Adapt</em>ensor
            </span>
            <span className="ml-1.5 text-[10px] font-mono bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded">
              ADMIN
            </span>
          </div>
        </Link>
      ...
      <div className="p-4 border-t border-brand-border space-y-3">
        ...
        <SignOutButton>
          <button className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-zinc-500 hover:text-red-400 hover:bg-red-500/5 transition-colors">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
